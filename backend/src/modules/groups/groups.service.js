...
export const deleteGroup = async (groupId, userId) => {
  ...
  if (group.avatar && group.avatar.includes('cloudinary')) {
    try {
      ...
      logger.warn(`Failed to delete group avatar from Cloudinary: ${error.message}`);
    }
  }
  ...
  logger.info(`Group deleted: ${groupId} by admin: ${userId}`);

  return { success: true };
};

export const transferAdmin = async (groupId, userId, newAdminId) => {
  const group = await Group.findById(groupId);
  if (!group) {
    throw new AppError('Group not found', 404);
  }
  ...
  if (group.admin.toString() !== userId) {
    throw new AppError('Only current admin can transfer admin', 403);
  }

  if (!group.members.includes(newAdminId)) {
    throw new AppError('New admin must be a member of the group', 400);
  }

  // Remove from moderators if they are one
  group.moderators = group.moderators.filter((m) => m.toString() !== newAdminId);
  ...
  logger.info(`Admin transferred in group: ${groupId} from ${userId} to ${newAdminId}`);

  return group;
};

export const demoteModerator = async (groupId, userId, targetUserId) => {
  const group = await Group.findById(groupId);
  if (!group) {
    throw new AppError('Group not found', 404);
  }
  ...
  if (group.admin.toString() !== userId) {
    throw new AppError('Only admin can demote moderators', 403);
  }

  group.moderators = group.moderators.filter((m) => m.toString() !== targetUserId);
  await group.save();

  logger.info(`Moderator demoted in group: ${groupId}`);
