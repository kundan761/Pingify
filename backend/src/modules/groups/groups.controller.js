...
export const createGroup = asyncHandler(async (req, res) => {
  // JSON strings are already parsed by parseFormData middleware
  const groupData = req.body;

  const group = await groupsService.createGroup(req.user.userId, groupData, req.file);
...
export const updateGroup = asyncHandler(async (req, res) => {
  ...
  const group = await groupsService.updateGroup(groupId, req.user.userId, updateData, req.file);
...
export const deleteGroup = asyncHandler(async (req, res) => {
  const { groupId } = req.params;
  await groupsService.deleteGroup(groupId, req.user.userId);
...
export const transferAdmin = asyncHandler(async (req, res) => {
  ...
  const group = await groupsService.transferAdmin(groupId, req.user.userId, newAdminId);
...
export const demoteModerator = asyncHandler(async (req, res) => {
  const { groupId, userId } = req.params;
  const group = await groupsService.demoteModerator(groupId, req.user.userId, userId);
